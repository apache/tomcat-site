...
<!-- Content Stylesheet for "tomcat-faq" Documentation -->
...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	  ...
	  <xsl:if test="$project-menu = 'menu'">
		  <tr>
			<td colspan="2" align='center'>
                          <xsl:comment>Google Search Form: see Bugzilla 35788</xsl:comment>
  ...
  <xsl:template match="question">
	<b style="font-size: larger"><xsl:apply-templates/></b>
  </xsl:template>

  <xsl:template match="answer">
	<div style="padding-left : 20px;"><xsl:apply-templates/></div><br />
  </xsl:template>
