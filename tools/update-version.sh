...
if [ "email" = "$5" ] ; then
  ...
  if [ "0" '!=' "$( expr "$osinfo" : '.*Darwin' )" ] ; then
    ...
    if [ "" = "$( command -v envsubst )" ] ; then
      open "mailto:Tomcat Developers List <dev@tomcat.apache.org>,Tomcat Users List <users@tomcat.apache.org>, announce@tomcat.apache.org, announce@apache.org?subject=[ANN] Apache Tomcat ${NEW_RELEASE} Available&body=$( cat tools/email-template-${MINOR_RELEASE}.txt | sed -e "s/\${NEW_RELEASE}/${NEW_RELEASE}/g" -e "s/\${OLD_RELEASE}/${OLD_RELEASE}/g" -e "s/\${RELEASE_MANAGER}/${RELEASE_MANAGER}/g" )&from=${RELEASE_MANAGER}@apache.org"
    else
      open "mailto:Tomcat Developers List <dev@tomcat.apache.org>,Tomcat Users List <users@tomcat.apache.org>, announce@tomcat.apache.org, announce@apache.org?subject=[ANN] Apache Tomcat ${NEW_RELEASE} Available&body=$( cat tools/email-template-${MINOR_RELEASE}.txt | envsubst )&from=${RELEASE_MANAGER}@apache.org"
    fi
  else
    echo Email template:
    echo
